module.exports = {
  ...
  async toolbox(req, res) {
    const {dbQuery} = req, text = await req.db.run(`${dbQuery}'toolboxText'`);
    res.status(200).send({text})
  },
  ...
  async actresses (req, res) {
    const {dbQuery, db: {run}} = req, text = await run(`${dbQuery}'actressesText'`)
    res.status(200).send({text})
  },

  async starWars (req, res) {
    const {dbQuery, db: {run}} = req, text = await run(`${dbQuery}'starWarsText'`)
    res.status(200).send({text})
  },
